...
export default class DragSortableView extends Component{
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
      ...
      if (prevState.originDataSource !== nextProps.dataSource) {
        ...
        return {
         ...
         itemHeight,
         dataSource,
         originDataSource: nextProps.dataSource,
         height: Math.ceil(dataSource.length/rowNum)*itemHeight
        }
      } else {
        return {
         height: Math.ceil(nextProps.dataSource.length/rowNum)*itemHeight,
         itemWidth,
         itemHeight
        }
      }
    ...
    startTouch(touchIndex) {
        ...
        if (this.sortRefs.has(touchIndex)) {
            ...
            Animated.timing(
                ...
                {
                    toValue: this.props.scaleNumber,
    ...
    moveTouch (nativeEvent,gestureState) {
        ...
        if (this.touchCurItem) {
            ...
            const rowNum = parseInt(this.props.parentWidth/this.state.itemWidth);
            const maxWidth = this.props.parentWidth-this.state.itemWidth
            const maxHeight = this.state.itemHeight*Math.ceil(this.state.dataSource.length/rowNum) - this.state.itemHeight
            ...
            let moveToIndex = 0
            let moveXNum = dx/this.state.itemWidth
            let moveYNum = dy/this.state.itemHeight
            if (moveXNum > 0) {
    ...
    changePosition(startIndex,endIndex) {
        ...
        this.setState({
            ...
            // 动画视图定位需要重绘
            this.state.dataSource.map((it,i)=> {
              it.position.setValue({
                 x: parseInt(it.originLeft+0.5),
                 y: parseInt(it.originTop+0.5),
              })
            })
        })
                ...
                <Animated.View
                    ...
                    style={[styles.item,{
                        ...
                        opacity: item.scaleValue.interpolate({
                            inputRange:[1,this.props.scaleNumber],
